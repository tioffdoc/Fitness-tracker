...
import { todayStr, addDays, dateNavLabel, formatDateNumeric, shortDateLabel, lastNDates, convert, fmt, movingAverage } from "../utils.js";
...
export function renderWeightTracking(container){
  ...
  const points = range.map(d=>{
    const dd = db.day(d);
    return { label: shortDateLabel(d, dateFormat), value: dd.weightKg==null?null:Math.round(convert.weightToDisplay(dd.weightKg,units.weight)*10)/10 };
  });
  ...
  container.innerHTML = `
    ...
    <div class="card">
      <div class="field">
        <label for="f-weight">Weight for ${formatDateNumeric(currentDate, dateFormat)}</label>
    ...
    <div class="card" style="padding:4px 14px;">
      ${history.length===0 ? `<div class="empty-state">${icon("scale")}<p>No entries yet</p></div>` : history.map(d=>{
        ...
        <div class="list-row">
          ...
          <div class="list-row__body"><div class="list-row__title">${formatDateNumeric(d, dateFormat)}</div></div>
  ...
  container.querySelector("#dateNavLabel").addEventListener("click", ()=>{
    openDateJumpSheet(currentDate, (newDate)=>{ currentDate = newDate; renderWeightTracking(container); });
  });
